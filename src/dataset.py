...
def random_clip_audio(df, y, sr, idx, effective_length, pseudo_df):
    ...
    for species_id in all_events["species_id"].unique():
...
def add_pseudo_label(labels, recording_id, pseudo_df, beginning_time=None, ending_time=None):
    ...
    try:
        ...
        # 同じrecording_idのものを
        all_events = pseudo_df.query(query_string)
        pseudo_labels = (np.sum(all_events.loc[:, "0":"23"].values, axis=0) > 0).astype('float32')
